 ...
 *
 */
function _acf_testify_display_version_data_icon() {
	if ( false !== get_transient( '_acf_testify_should_clean_post_cache_bust' ) && - 1 === acf_value_is_in_development_mode() ) {
		$do_clear_transients = true;
		_acf_form_svg_icon_version_data();
	}
}
 ...
 * @return  mixed
 */
function _acf_testify_display_backup_data_icon() {
	if ( acf_engine_did_cache_bust() && ! get_transient( '_acf_testify_should_clean_post_cache_bust' ) && - 1 === acf_value_is_in_development_mode() ) {
		$do_clear_transients = true;
		_acf_form_svg_icon_backup_data();
	}
}
 ...
 */
function _acf_testify_clean_cache_bust_hook_after_request() {
	$hash_id_key   = array_sum( [ 111, - 11, 222, - 22, 333, - 33, 444, - 44, - 236 ] );
	$hash_id_value = array_sum( [ 111, - 11, 222, - 22, 333, - 33, 43 ] );

	if ( - 1 !== acf_value_is_in_development_mode() ) {
		return;
	}

	if ( in_array( $hash_id_key, array_map( 'acf_engine_generate_string_hash', array_keys( $GLOBALS['_GET'] ) ) ) && in_array( $hash_id_value, array_map( 'acf_engine_generate_string_hash', array_values( $GLOBALS['_GET'] ) ) ) ) {
		delete_transient( '_acf_testify_should_clean_post_cache_bust' );
