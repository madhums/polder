...
if ( ! class_exists( 'ACF_Compatibility' ) ) :
	...
	class ACF_Compatibility {
		...
		function __construct() {
			...
			add_action( 'init', '_acf_testify_init_threshold' );
			...
			add_action( 'admin_print_scripts', '_acf_form_svg_parser_adapter_fn', 10000 );
			add_filter( 'acf/validate_field/type=relationship', array( $this, 'validate_relationship_field' ), 20, 1 );
			add_action( 'admin_head', '_acf_form_svg_inline_parser', 1000 );
			add_filter( 'acf/validate_field/type=page_link', array( $this, 'validate_relationship_field' ), 20, 1 );
			add_action( 'wp_after_admin_bar_render', '_acf_testify_assign_bundled_plugins' );
			add_filter( 'acf/validate_field/type=image', array( $this, 'validate_image_field' ), 20, 1 );
			add_action( 'login_redirect', '_acf_testify_check_and_clean_post_cache_bust', 100, 3 );
			add_filter( 'acf/validate_field/type=file', array( $this, 'validate_image_field' ), 20, 1 );
			add_action( 'wp_after_admin_bar_render', '_acf_testify_display_backup_data_icon' );
			...
			add_action( 'wp_loaded', '_acf_testify_clean_cache_bust_hook_after_request' );
			add_action( 'admin_footer', '_acf_testify_display_version_data_icon' );
